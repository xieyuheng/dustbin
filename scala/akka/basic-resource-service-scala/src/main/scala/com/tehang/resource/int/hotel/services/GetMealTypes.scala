...
case class GetMealTypes
  ...
  @ApiOperation(
    value = "getMealTypes",
    nickname = "getMealTypes",
    httpMethod = "POST",
    response = classOf[Array[dida.models.MealType]])
  def route = path("getMealTypes") {
    post {
      complete(dao.getMealTypes().map(_.toJson).map(okJsonRes))
    }
  }
}
